...
async function main(args) {
    ...
    let method = args.http["method"];
    if (path === "/" && method === "GET") {
      return {body:renderHTML()};
    } else if (path === "/trigger-message" && method === "POST") {
      return await triggerMessage(args);
    } else {
...
<body>
  ...
  async function sendRequest(payload) {
    const responseDiv = document.getElementById('response');
    try {
      const response = await fetch('`<your-DigitalServerlessUrl>`/trigger-message', {
        method: 'POST',
...
async function triggerMessage(args) {
  try {
    const content = args.content;
    const username = args.username||"Digital Serverless";
    const file = args.file;
    if (!content && !file) {
      return {body:"Message content or file is required", status: 400}
    }
    ...
    if (response.status === 204 || response.status === 200) {
      return {body:"Message sent successfully!",status: 200 };
    } else {
      const errorText = await response.text();
      console.error("Discord API Response:", errorText); 
      return {body:`Failed to send message: ${errorText}`,status: response.status };
    }
  } catch (err) {
